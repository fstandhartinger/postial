...
export function TeamForm({ action, target, role = 'editor', disabled = false }: { action: string; target?: string; role?: string; disabled?: boolean }) {
  ...
  return <form action={submit} className="flex flex-wrap items-center gap-3">
    ...
    {state.link && <div className="w-full min-w-0 space-y-2 rounded bg-emerald-50 p-3"><p>Copy this link now. It will not be shown again. Anyone with this link can join. Expires in 7 days.</p><input aria-label="Invite link" readOnly value={state.link} className="w-full rounded border p-2"/><Button type="button" onClick={async () => { try { await navigator.clipboard.writeText(state.link!); setCopied(true); } catch { setCopied(false); } }}>{copied ? 'Copied' : 'Copy'}</Button></div>}
  </form>;
}
