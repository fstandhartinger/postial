...
export async function ApprovalPanel({
  ...
  const history = await getDb()
    ...
    .where(eq(approvalDecisions.postId, post.id))
  ...
  return (
    <Card>
      ...
      {origin && post.approvalToken ? (
        ...
        <p>Set NEXT_PUBLIC_APP_URL to share approval links.</p>
      )}
      {canReview(post.status) && (
        <form
          className="mt-4"
          action={regenerateApprovalLink.bind(null, post.id)}
        >
          <Button>Regenerate link</Button>
        </form>
      )}
      <ol className="mt-4 space-y-3">
        {history.map((d) => (
          <li key={d.id}>
            <p>
              {d.reviewerName} {d.decision.replaceAll("_", " ")} ·{" "}
              {d.createdAt.toLocaleString("en-GB", { timeZone: "UTC" })} UTC
            </p>
            {d.comment && (
              <p className="whitespace-pre-wrap break-words">{d.comment}</p>
            )}
          </li>
        ))}
      </ol>
