...
export function ApiForm({kind, id}: {kind: 'create_key' | 'create_webhook' | 'revoke' | 'test_webhook' | 'disable_webhook'; id?: string}) {
  const [state, action, pending] = useActionState(settingsAction, {} as SettingsState);
  const labels = {create_key: 'Create API key', create_webhook: 'Create webhook', revoke: 'Revoke', test_webhook: 'Send test event', disable_webhook: 'Disable webhook'};
