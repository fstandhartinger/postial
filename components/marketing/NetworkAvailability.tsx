...
export function NetworkAvailability({ source }: { source?: 'pricing' | 'roadmap' }) {
  return <div>{(['live', 'preparation', 'planned'] as const).map(status => {
    const networks = availability.networks.filter(network => network.status === status);
    if (!networks.length) return null;
    return <section className="my-6" key={status}><h3>{status === 'live' ? 'Live' : status === 'preparation' ? 'Early access connections' : 'Planned'}</h3><div className="marketing-grid three">{networks.map(network => {
