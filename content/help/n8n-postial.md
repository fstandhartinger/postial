...
The API and the native node require an active Agency plan or Agency trial. Create a named API key in Postial’s [API settings](/app/settings/api), copy it when it is shown, and grant only the scopes the workflow needs: The key is shown once when it is created, so copy it immediately; it cannot be displayed again.
