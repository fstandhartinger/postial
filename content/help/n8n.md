...
2. Create a SocialMint API key with the required scopes in [API settings](/app/settings/api).
...
2. Register that URL in SocialMint API settings and choose events such as post.published or approval.decided.
...
4. Activate the workflow, send a SocialMint test event and inspect both delivery logs and the n8n execution.

## Native node installation status

The native package is named n8n-nodes-socialmint and awaits npm publication. It is not currently offered as an installable public release. After publication, an n8n owner whose installation permits community packages can install that package through Settings → Community Nodes → Install, then search for SocialMint or SocialMint Trigger. n8n Cloud discovery additionally requires n8n verification. Configure SocialMint API credentials with the base URL https://socialmint.app.mintapis.com (without /api/v1) and your API key. Grant brands:read for dropdowns, posts:read for reads, posts:write for changes and webhooks:manage for automatic trigger registration. Until then, use the HTTP Request and Webhook nodes above. No release date is promised; contact [support](mailto:info@productivity-boost.com) for release availability.

## Native trigger setup after publication

1. Add SocialMint Trigger and choose one or more events, for example Approval Decided.
...
3. Alternatively, choose Manual (Paste Secret), register the production URL yourself in SocialMint and paste its signing secret. Disable or delete that endpoint yourself on deactivation.
4. Test visible workflow output with an approval change request. The native trigger acknowledges SocialMint’s ping test but intentionally emits no workflow item for it.
