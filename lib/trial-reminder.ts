...
type ReminderKind = 'trial_will_end' | 'payment_failed';
...
function content(kind: ReminderKind, trialEnd: Date | null): { subject: string; text: string; html: string } {
  ...
  const text = kind === 'trial_will_end'
  ...
  const heading = kind === 'trial_will_end' ? 'Your Postial trial is ending' : 'Postial payment could not be processed';
