...
function content(kind: ReminderKind, trialEnd: Date | null, workspace?: string): { subject: string; text: string; html: string } {
  ...
  // say which one stopped publishing. The incident mail has named its channel from the start.
  const where = workspace?.trim() ? ` for ${workspace.trim().slice(0, 60)}` : '';
  const heading = kind === 'trial_started' ? `Your Postial trial${where} is running`
    : kind === 'trial_ended' ? `Your Postial trial${where} has ended`
    : kind === 'trial_will_end' ? `Your Postial trial${where} is ending` : `Postial payment${where} could not be processed`;
...
export async function sendSubscriptionReminder(subscriptionId: string, kind: ReminderKind): Promise<void> {
  ...
  try {
    const [row] = await db.select({ email: users.email, trialEnd: subscriptions.trialEnd, workspace: workspaces.name })
      .from(subscriptions)
    ...
    const mail = content(kind, row.trialEnd, row.workspace ?? undefined);
