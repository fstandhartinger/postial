...
function validateRuntimeConfig() {
  ...
  // Verification-only shortcuts (e.g. publisher fixtures) must never be reachable on a public origin.
  // The verification suite runs a production build against loopback, so key on the host, not NODE_ENV.
  if (process.env.VERIFY_MODE === '1' && origins.some(o => !['localhost','127.0.0.1','[::1]'].includes(new URL(o).hostname)))
    throw new Error('VERIFY_MODE must not be set on a public origin');
