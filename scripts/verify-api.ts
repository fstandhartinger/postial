...
import { decideApproval, publicApproval } from '../lib/approvals';
...
async function main() {
  ...
  try {
    ...
    const approvalPage = await publicApproval(new URL(approval.approval_url).pathname.split('/').pop()!);
    assert(approvalPage);
    assert.equal((await decideApproval(new URL(approval.approval_url).pathname.split('/').pop()!, {reviewerName: 'Tester', decision: 'approved', comment: '', approvalVersion: approvalPage.approvalVersion}, '127.0.0.1')).status, 200);
