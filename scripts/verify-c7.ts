...
async function main() {
  ...
  try {
    ...
    const healthData=await h.json();assert(healthData.migrations.applied>=17);assert.match(healthData.migrations.latest,/0017/);assert('lastTickAt' in healthData.worker);assert.equal(typeof healthData.errorsLastHour,'number');
