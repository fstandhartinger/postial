...
try {
  ...
  for (let i = 1; i < 9; i++) assert.equal((await send(`${run}-${i}@example.com`)).status, 202);
  ...
  console.log('PASS: valid duplicates consume quota, duplicate remains generic at limit, proxy spoof resistance, persisted rows');
} finally { await db`delete from network_waitlist where email like ${run+'-%'}`; await db`delete from request_rate_limits where key like 'waitlist:attempt:%'`; await db.end(); }
