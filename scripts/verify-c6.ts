...
// Derived from the copy file so every comparison page is covered here; a new page must not silently escape this check.
import compareCopy from '../content/compare.json';
  ...
  try{
    ...
    if(process.env.C6_HTTP_URL){
      ...
      for(const width of [390,1280]){
        ...
        for(const path of ['/','/pricing',...compareCopy.pages.map((page:{slug:string})=>`/compare/${page.slug}`)]){await page.goto(base+path);const box=page.locator('[data-availability]').first();await box.waitFor();const availabilityText=await box.innerText();assert.match(availabilityText,/Bluesky, Mastodon, Telegram/);assert.match(availabilityText,process.env.SMTP_URL?/Magic links are available/ : /Sign-in is not configured/);assert(!(await page.locator('.feature-list').allTextContents()).some((s:string)=>/✓[^✓]*(?:\bX\b|Threads|LinkedIn)/.test(s)));}
      }
      ...
      await db.delete(dpaAcceptances).where(eq(dpaAcceptances.workspaceId,workspace.id));await page.goto(base+'/app/settings/legal');await Promise.all([page.waitForResponse((r: { request(): { method(): string } })=>r.request().method()==='POST'),page.getByRole('button',{name:'Accept DPA',exact:true}).click()]);await page.getByRole('heading',{name:'DPA accepted'}).waitFor();assert.equal((await db.select().from(dpaAcceptances).where(eq(dpaAcceptances.workspaceId,workspace.id)))[0].userId,userId);await page.screenshot({path:evidence+'/dpa-accepted.png',fullPage:true});assert.deepEqual(errors,[]);
