  ...
  try{
    ...
    if(process.env.C6_HTTP_URL){
      ...
      for(const width of [390,1280]){
        ...
        for(const path of ['/','/pricing','/compare/hootsuite-alternative','/compare/postiz-alternative']){await page.goto(base+path);const box=page.locator('[data-availability]').first();await box.waitFor();const availabilityText=await box.innerText();assert.match(availabilityText,/Bluesky, Mastodon, Telegram/);assert.match(availabilityText,/Google sign-in and magic links are available/);assert(!(await page.locator('.feature-list').allTextContents()).some((s:string)=>/✓[^✓]*(?:\bX\b|Threads|LinkedIn)/.test(s)));}
      }
