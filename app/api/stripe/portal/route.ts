...
export async function POST(request: Request) {
  try {
    ...
    const session = await stripe().billingPortal.sessions.create({ customer: subscription.stripeCustomerId, configuration: requiredEnv('STRIPE_PORTAL_CONFIG'), return_url: `${appUrl()}/app/billing` });
