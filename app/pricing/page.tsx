...
export default async function Pricing() { await connection(); await recordPublicView('pricing_view', '/pricing'); const faqSchema = { '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: faqEntries(true).map(entry => ({ '@type': 'Question', name: entry.question, acceptedAnswer: { '@type': 'Answer', text: entry.answer } })) }; return <div className="pricing-page"><ClientBeacon path={'/pricing'} /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(faqSchema) }} /><header><Badge>Early access</Badge><p className="eyebrow">Pricing</p><h1>{words('Pricing', 'H2')}</h1><p>{words('Pricing', 'Sub')}</p><p>14-day free trial, no card needed</p></header><AccessStatus /><Plans checkout /><section className="section"><h2>Which networks do your clients need?</h2><NetworkAvailability source="pricing" /></section><section className="section faq-section"><h2>{words('FAQ', 'H2')}</h2><FAQ short /></section></div>; }
