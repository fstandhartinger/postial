...
export default async function CheckEmail({ searchParams }: { searchParams: Promise<{ email?: string; limited?: string }> }) {
  ...
  return <Card className="mx-auto max-w-md"><h1 className="text-3xl font-semibold">Check your email</h1>
    ...
    {params.limited === "1" && <p role="status" className="mt-3 text-sm text-gray-600">Please wait a little while before requesting another link. For privacy, this message is the same whether or not an address exists.</p>}
