...
export default async function FunnelPage() {
  ...
  return <section><h1 className="text-3xl font-semibold">Funnel</h1><p className="text-sm text-zinc-600">Last {report.days} days, from {report.since}</p>
    ...
    <h2 className="mt-6 text-xl font-semibold">Totals by client class</h2>{CLIENT_CLASSES.map(clientClass => <section key={clientClass} className="mt-4"><h3 className="text-lg font-medium">{clientClass === 'browser' ? 'People (browser)' : clientClass === 'internal' ? 'Internal (staff and agents, excluded)' : clientClass}</h3><table className="mt-2"><tbody>{FUNNEL_EVENTS.map(e => <tr key={e}><th className="p-2 text-left">{e}</th><td className="p-2">{report.clientClassTotals[clientClass][e] ?? 0}</td></tr>)}</tbody></table></section>)}
