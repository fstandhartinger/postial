...
export default async function ApprovalsPage({searchParams}:{searchParams:Promise<{brand?:string}>}) {
  ...
  if (!access.approvalLinks) return <><h1>Approvals</h1><Card><h2>Client approvals are included with Agency</h2><p>Collect feedback and approve posts before publishing.</p><Link className="underline" href="/app/billing">Upgrade to Agency</Link></Card></>;
  ...
  return <><h1>Approvals</h1><p className="text-zinc-600">Client decisions across your brands. Drafts await sharing; publishing progress appears under Approved.</p>
