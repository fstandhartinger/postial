...
export default function Home() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, '\\u003c') }} />
    ...
    <section className="section networks"><h2>{words('Networks', 'H2')}</h2><p className="network-list">{availability.channels.join(', ')}</p><p><strong>{words('Networks', 'Roadmap label')}</strong></p><p>{availability.planned.join(', ')} — in preparation, subject to platform approvals, with no release date.</p></section>
