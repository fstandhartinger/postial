...
export default function Home() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, '\\u003c') }} />
    ...
    <section className="section demo-section" id="demo"><h2 id="demo-heading" tabIndex={-1}>{words('Interactive demo', 'H2')}</h2><p>{words('Interactive demo', 'Sub')}</p><LazyApprovalDemo /></section>
    ...
    <section className="section final-cta"><h2>{words('Final CTA', 'H2')}</h2><p>{words('Final CTA', 'Body')}</p><Link prefetch={false} className="primary" href="/login">{words('Final CTA', 'CTA → /signup')}</Link></section>
  </>;
}
