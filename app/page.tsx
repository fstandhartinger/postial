...
export default function Home() {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, '\\u003c') }} />
    ...
    <section className="section"><h2>{words('How it works', 'H2')}</h2><ol className="grid three steps">{[1, 2, 3].map(step => <li key={step}><h3>{words('How it works', `Step ${step} label`)}</h3><p>{words('How it works', `Step ${step} body`)}</p></li>)}</ol></section>
    <section className="section demo-section" id="demo"><h2 id="demo-heading" tabIndex={-1}>{words('Interactive demo', 'H2')}</h2><p>{words('Interactive demo', 'Sub')}</p><ApprovalDemo /></section>
    <section className="section networks"><h2>{words('Networks', 'H2')}</h2><p className="network-list">{words('Networks', 'Available labels')}</p><p><strong>{words('Networks', 'Roadmap label')}</strong></p><p>{words('Networks', 'Roadmap body')}</p></section>
    <section className="section" id="pricing"><h2>{words('Pricing', 'H2')}</h2><p>{words('Pricing', 'Sub')}</p><Plans /><Link className="text-link" href="/pricing">Pricing →</Link></section>
    ...
    <section className="section final-cta"><h2>{words('Final CTA', 'H2')}</h2><p>{words('Final CTA', 'Body')}</p><Link className="primary" href="/login">{words('Final CTA', 'CTA → /signup')}</Link></section>
  </>;
}
